...
SELECT
  ...
  CASE
    ...
    WHEN (NOW()::timestamptz AT TIME ZONE 'America/Sao_Paulo')::time NOT BETWEEN p.hora_inicio AND p.hora_fim THEN
            CASE
                -- Condição de Alerta:
                ...
                OR (NOW()::timestamptz - u."timestamp" > p.limite_tolerado)
                THEN 1
